						...
						<div class="row-fluid">
							<div class="span12">
								...
								foreach($allrows as $row){
									...
									echo '<div class="col-xs-12 col-sm-9">';
									echo '<div class="clearfix">';
									echo '<input type="text" name="username" id="username"  class="col-xs-12 col-sm-6"  value="'.$row['username'].'"/>';
									...
									echo '<input type="text"  class="col-xs-12 col-sm-6"  id="password" name="password" value="'.$password.'"/>';
									echo '</div>';
									echo '</div>';
									echo '</div>';
									echo '<div class="form-group">';
									...
									echo '<div class="col-xs-12 col-sm-9">';
									echo '<div class="clearfix">';
									echo '<input type="text"  class="col-xs-12 col-sm-6" id="usertype" name="usertype"  disabled value="'.$usertype.'"/>';
									echo '</div>';
									echo '</div>';
									echo '</div>';
									echo '<div class="form-group">';
									...
									echo '<div class="col-xs-12 col-sm-9">';
									echo '<div class="clearfix">';
									echo '<input type="email" id="email" name="email" class="col-xs-12 col-sm-6"   value="'.$row['email'].'"/>';
									echo '</div>'.'</div>'.'</div>';
									...
									echo '<div class="col-xs-12 col-sm-9">';
									echo '<div class="clearfix">';
									echo '<input type="text" id="mobilephone" name="mobilephone" class="col-xs-12 col-sm-6"  value="'.$row['mobilephone'].'" />';
									echo '</div>.</div>.</div>';
								...
								?>
<!--  echo '<a  class="btn btn-xs btn-info"  role="button" href="../controller/delete.php?username='.$row['username'].'" onclick="javascript:return p_del()"><i class="icon-remove bigger-120">删 除</i></a>';-->
					<div class="space-8"></div>
								<div class="row-fluid wizard-actions col-xs-12 col-sm-6 no-padding-right">
									<span id="tablename"  hidden="hidden"><?php echo $table; ?></span>
									<span id="preusername" hidden="hidden"><?php echo $row['username']; ?></span>
									<a  href="javascript:;" onclick="getVal()" class='btn btn-success btn-next'  role='button'>提交</a>
									<label id="middleValue" style="color:red;"></label>
								</div>
							</div><!-- /widget-main -->
...
<script>
	function getVal(){
		...
		request.open("POST", "../controller/save.php");
		var data = "usernamePass=" + document.getElementById("username").value
				+ "&passwordPass=" + document.getElementById("password").value
				+ "&emailPass=" + document.getElementById("email").value
				+ "&mobilephonePass=" + document.getElementById("mobilephone").value
				+ "&tablename=" + document.getElementById("tablename").innerHTML
				+ "&preusername=" + document.getElementById("preusername").innerHTML;
		request.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		request.send(data);
		request.onreadystatechange = function() {
			if (request.readyState===4) {
				if (request.status===200) {
					document.getElementById("middleValue").innerHTML = request.responseText;
					window.setTimeout("location.href='account.php'",1000);
				} else {
